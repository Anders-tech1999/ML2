...
vip(svm_model, method = "permute", nsim = 5, train = baked_train, 
    target = "X0", metric = "auc", reference_class = "yes", 
    pred_wrapper = prob_yes)

# We find that the variables that has the most predictive ability on whether a client has subscribed a term deposit are:
# X20 (Number of employees)
# X13 (days since last contact to client)
# X15 (Previous outcome of campaign = success)
# X17 (Consumer Confidence Index)

#construct PDP (feature effect plots are on the probability scale)
features <- c("X20", "X13", 
              "X15_success", "X18")
pdps <- lapply(features, function(x) {
  partial(svm_model, pred.var = x, which.class = 2,        # since the predicted probabilities from our model come in two columns (No and Yes), we specify which.class = 2 so that our interpretation is in reference to predicting Yes
