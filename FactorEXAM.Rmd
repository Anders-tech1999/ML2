...
```{r}
library(tree)
oj.tree = tree(HighratingOverallBinary ~ ., subtrain)
summary(oj.tree)
...
table(subtest$HighratingOverallBinary, oj.pred)
...
table(subtest$HighratingOverallBinary, pred.unpruned)
...
table(subtest$HighratingOverallBinary, pred.pruned)
...
# Random Forest
# number of features
n_features <- length(setdiff(names(subtrain), "HighratingOverallBinary"))
n_features #13
...
p <- predict(OptimalModel, subtest)
pred <- p$predictions
...
# Confusion Matrix
confusionMatrix(
  pred, subtest$HighratingOverallBinary, positive = "1")
...
gbm_pred <- predict(gbmfit, newdata = subtest)

# Create a confusion matrix
confusion_mat <- confusionMatrix(gbm_pred, subtest$HighratingOverallBinary)
...
predicted_probs <- predict(gbmfit, newdata = subtest, type = "prob")

# Extract the predicted probabilities for the positive class
positive_probs <- predicted_probs[, "1"]

# Create a binary outcome vector (1 for positive class, 0 for negative class)
outcome <- ifelse(subtest$HighratingOverallBinary == "1", 1, 0)

# Calculate the ROC curve and AUC
library(caTools)
roc_obj <- roc(outcome, positive_probs)
plot(roc_obj)
colAUC(predicted_probs[ , 1], as.character(subtest$HighratingOverallBinary), plotROC = T)
...
# Confusion Matrix
predicted_val <- predict(gbmfit, subtest)
confusionMatrix(
  predicted_val, subtest$HighratingOverallBinary, positive = "1")
...
xgbcaret <- train(HighratingOverallBinary ~ ., subtrain,
                     method = "xgbTree",
...
predicted_probsXG <- predict(xgbcaret, newdata = subtest, type = "prob")

# Extract the predicted probabilities for the positive class
positive_probsXG <- predicted_probsXG[, "1"]

# Create a binary outcome vector (1 for positive class, 0 for negative class)
outcomeXG <- ifelse(subtest$HighratingOverallBinary == "1", 1, 0)

# Calculate the ROC curve and AUC
roc_objXG <- roc(outcomeXG, positive_probsXG)
roc_objXG
colAUC(predicted_probsXG[ , 1], as.character(subtest$HighratingOverallBinary), plotROC = T) #0.7543
...
# Confusion Matrix
predicted_val <- predict(xgbcaret, subtest)
confusionMatrix(
  predicted_val, subtest$HighratingOverallBinary, positive = "1")
...
vip::vip(churn_svm_auc, method = "permute", event_level = "second", nsim = 5, train = subtrain, 
    target = "HighratingOverallBinary", metric = "roc_auc", reference_class = "X1", 
    pred_wrapper = prob_0)
```
...
features <- c("ratingCareerOpportunities", "ratingCultureAndValues", 
              "ratingCompensationAndBenefits", "ratingRecommendToFriend")
